...
def search_2_snp(vcf_file):
    ...
    chrom_pre = pos_pre = ref_pre = alt_pre = "" #modified
    snps_list = list()
...
if __name__ == "__main__":
    ...
    if len(snps_list) == 0:
        print("There is no two adjacent SNPs in the input VCF file.\nNo DNP combination was did, but DNP INFO and FILTER headers were added to the output file: {}".format(args.output_file), file=sys.stderr)
        ...
        if len(dnps_list) == 0:
            print("The following two adjacent SNPs are found in the input VCF file: {}.\nNo two adjacent SNPs passed the threshold: {}.\nNo DNP combination was did, but DNP INFO and FILTER headers were added to the output file: {}".format(snps_list, args.threshold_value, args.output_file), file=sys.stderr)
            combine_2_snp([], args.vcf_file, args.output_file, args.threshold_value)
        else:
            print("The following two adjacent SNPs are found in the input VCF file: {}.\nThe following two adjacent SNPs: {} passed the threshold: {}.\nPassed two adjacent SNPs are combined to DNPs, and DNP INFO and FILTER headers were added to the output file: {}".format(snps_list, dnps_list, args.threshold_value, args.output_file), file=sys.stderr)
